...
const packageNames: Record<string, string> =
  Object.fromEntries(
    PROJECT_PACKAGES.map((pkg) => [pkg.id, pkg.name])
  );
...
function packageName(
  id: ProjectPackage | null | undefined
): string {
  if (!id) {
    return "Not selected";
  }
...
function getNextProjectStage(status: ProjectStatus) {
  const index = PROJECT_PIPELINE.findIndex(
  ...
  return PROJECT_PIPELINE[index + 1];
}

/* -------------------------------------------------------------------------- */
/* MAIN PAGE                                                                  */
/*                                                                            */
/* The OS shell/sidebar is owned by app/os/layout.tsx.                       */
...
export default function LbmOsPage() {
  ...
  const refresh = useCallback(() => {
    ...
    try {
      ...
      console.error("LBMOS refresh failed:", error);
    } finally {
      window.setTimeout(() => {
        setRefreshing(false);
      }, 250);
    ...
    () =>
      projects.reduce((total, project) => {
        ...
        return (
          total +
          Math.max(
            project.balanceDue - project.depositPaid,
            0
          )
        );
      }, 0),
  ...
  function qualifyLead(lead: Lead) {
  ...
  function convertClient(lead: Lead) {
  ...
  function createProjectFromLead(lead: Lead) {
    ...
    const selectedPackage: ProjectPackage =
      lead.packageInterest ?? "little_brush";

    const investment = packagePrice(selectedPackage);
    ...
    const project = createProject({
      ...
      priority: "standard",
      projectAddress:
        client.address ?? lead.location,
      creativeBrief:
        lead.enquiryMessage || undefined,
      wallCount: packageWallCount(selectedPackage),
  ...
  function advanceProject(project: Project) {
    const nextStage = getNextProjectStage(project.status);
  ...
  /* RENDER                                                                    */
  /*                                                                            */
  /* IMPORTANT: no <main className="lbmOs">, no <aside className="osSidebar">, */
  /* and no <section className="osMain"> here. The parent layout owns them.   */
  /* ------------------------------------------------------------------------ */
    ...
    <>
      <header className="osHeader">
        <div>
          ...
          <h1>
            {view === "dashboard" && "Command Centre"}
            {view === "leads" && "Lead Inbox"}
        ...
        <section>
          <div className="osSectionIntro">
            <div>
              ...
              <h2>
                Families you've converted
              </h2>
            </div>

            <strong>
              {clients.length} families
            </strong>
          </div>
        ...
        <section>
          <div className="osSectionIntro">
            <div>
              ...
              <h2>
                Every Childhood Project in motion
              </h2>
            </div>
    ...
    <>
      <div className="metricGrid">
        ...
        <Metric
          ...
          value={money.format(depositsCollected)}
      ...
      <div className="dashboardGrid">
        ...
        <section className="osPanel">
          <div className="panelHeader">
            <div>
              <p className="osEyebrow">REVENUE</p>
...
function LeadCard({
  ...
  return (
    <button
      type="button"
      className={`leadCard ${selected ? "selected" : ""}`}
      onClick={onClick}
    >
...
function LeadDetail({
  ...
  if (!lead) {
    return (
      <aside className="detailPanel emptyDetail">
        ...
        <p>
          Select an enquiry from the inbox to qualify,
          convert and create a project.
        </p>
      </aside>
    );
  }
  ...
  const existingClient = clients.find(
    (client) => client.leadId === lead.id
  );

  const existingProjectText = existingClient
  ...
  return (
    <aside className="detailPanel">
      <div className="detailHeader">
        <div>
          <p className="osEyebrow">LEAD</p>

          <h2>{lead.parentName}</h2>
      ...
      <div className="detailFacts">
        ...
        <Fact
          icon={<MapPin size={16} />}
          label="Location"
          value={lead.location || "Not supplied"}
        />

        <Fact
          icon={<BriefcaseBusiness size={16} />}
          label="Package"
          value={packageName(lead.packageInterest)}
        />
      </div>
      ...
      <div className="detailBlock">
        ...
        <p>
          {lead.enquiryMessage || "No enquiry notes."}
        </p>
      </div>
...
function ClientCard({
  ...
  const initials =
    client.parentName.trim().charAt(0).toUpperCase() ||
    "?";
...
function ProjectDetail({
  ...
  if (!project) {
    return (
      <section className="projectDetail emptyDetail">
        ...
        <p>
          Select a project above to manage its commercial
  ...
  const stageIndex = PROJECT_PIPELINE.findIndex(
  ...
  const depositRequired = project.depositRequired ?? 0;

  const balanceRemaining =
    project.balanceDue !== null
      ? Math.max(
          project.balanceDue - project.depositPaid,
          0
        )
  ...
  return (
    <section className="projectDetail">
      <div className="projectDetailHeader">
        <div>
          <p className="osEyebrow">PROJECT CONTROL</p>

          <h2>
            {project.childName}'s Childhood Project
          </h2>

          <span>
            {client?.parentName ?? "Client"} ·{" "}
            {packageName(project.packageId)}
          </span>
        </div>
      ...
      <div className="projectDetailStats">
        ...
        <div>
          ...
          <strong>
            {project.depositRequired !== null
              ? money.format(depositRequired)
              : "Quote"}
          </strong>
        </div>
        ...
        <div>
          ...
          <strong>
            {money.format(project.depositPaid)}
          </strong>
        </div>
      ...
      <div className="projectDetailMeta">
        <Fact
          ...
          value={
            project.projectAddress || "Not supplied"
          }
        />
        ...
        <Fact
          icon={<BriefcaseBusiness size={16} />}
          label="Package"
          value={packageName(project.packageId)}
        />
      ...
      {project.creativeBrief && (
        <div className="detailBlock">
          ...
          <p>{project.creativeBrief}</p>
        </div>
      )}

      <div className="projectProgress">
        {PROJECT_PIPELINE.map((stage, index) => (
          <div
